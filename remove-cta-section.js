...
const files = fs.readdirSync(publicDir)
  .filter(f => f.endsWith('.html') && f !== '.html');
...
files.forEach(file => {
  ...
  const sectionStart = '<section class="cta">';
  const sectionEnd = '</section>';

  const startIdx = content.indexOf(sectionStart);
  if (startIdx === -1) {
    console.log(`${file}: No CTA section found - skipping`);
    return;
  }
  ...
  console.log(`✓ Removed CTA section from ${file}`);
});
